...
CLASS ycl_addict_class_inheritance IMPLEMENTATION.
  METHOD get_cache.
    ...
    """""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
    TRY.
        result = REF #( me->cache[ KEY primary_key COMPONENTS clsname = clsname ] ).
  ...
  METHOD get_instanceable_subclasses.
    ...
    IF cache->insta_subcnam_read = abap_false.
      ...
      DATA(clsname_rng) = VALUE ycl_addict_class=>clsname_range( FOR cn IN cache->insta_subcnam
      ...
      SELECT clsname FROM seoclassdf AS sd1
             WHERE clsname    IN @clsname_rng
               AND version     > 0
               AND version     = ( SELECT MAX( version ) FROM seoclassdf AS sd2
                                          WHERE clsname = sd1~clsname )
               AND clsabstrct  = @abap_true
             ORDER BY clsname
      ...
      LOOP AT cache->insta_subcnam ASSIGNING FIELD-SYMBOL(<clsname>).
        READ TABLE abstracts TRANSPORTING NO FIELDS
        ...
        READ TABLE interfaces TRANSPORTING NO FIELDS
             WITH KEY clsname = <clsname>
             BINARY SEARCH.
  ...
  METHOD get_recursive_subclasses.
    ...
    IF cache->recur_subcnam_read = abap_false.
      get_recursive_subclass_names_p( EXPORTING refclsname = parent
                                                rec        = abap_false
  ...
  METHOD get_recursive_subclass_names_p.
    ...
    LOOP AT clsname_local ASSIGNING FIELD-SYMBOL(<cl>).
      ...
      get_recursive_subclass_names_p( EXPORTING refclsname = <cl>
                                                rec        = abap_true
